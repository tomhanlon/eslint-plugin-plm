...
module.exports = {
  ...
  create: (context) => {
    ...
    function isReturnStatement(node) {
      return node.type === 'ReturnStatement' && _.has(node, 'argument.arguments');
    }
    ...
    function recordSinonInstance(variableDeclarer) {
      if (variableDeclarer.init.type === 'CallExpression' &&
        _.get(variableDeclarer, 'init.callee.object.name') === 'sinon' &&
        _.has(variableDeclarer, 'init.arguments') &&
        ...
        _.get(variableDeclarer, 'init.callee.type') === 'MemberExpression' &&
        _.get(variableDeclarer, 'init.callee.object.callee.object.name') === 'sinon' &&
    ...
    function nodeHasExpect(node) {
      return node.type === 'ExpressionStatement' && node.expression &&
        _.get(node, 'expression.callee.object.object.callee.name') === 'expect';
    }
    ...
    function isStubRestore(node) {
      return node.type === 'ExpressionStatement' &&
        _.get(node, 'expression.callee.property.name') === 'restore';
    }

    function checkForSinonRestore(node) {
      if (_.has(node, 'body.body.length')) {
        const bodyExpressions = node.body.body;
        bodyExpressions.forEach((expression) => {
          if (expression.type === 'ExpressionStatement' &&
            _.get(expression, 'expression.callee.property.name') === 'restore'
          ) {
            const sinonVariable = _.get(expression, 'expression.callee.object.name');
    ...
    function recordSinonAssignment(node) {
      ...
      if (_.has(node, 'body.body.length')) {
        node.body.body.forEach((expressionNode) => {
          const possibleSinonMethod = _.get(expressionNode, 'expression.right.callee.property.name');
          const variableBoundToSinonInstance = _.get(expressionNode, 'expression.left.name');

          if (DANGEROUS_SINON_METHODS.includes(possibleSinonMethod)) {
    ...
    function lintTestHookExpression(bodyBlockNode) {
      ...
      const mochaHookArgs = _.get(bodyBlockNode, 'expression.arguments');
      const mochaHookName = _.get(bodyBlockNode, 'expression.callee.name');
      if (!mochaHookName || !mochaHookArgs || !hookMethods.includes(mochaHookName)) { return; }

      lintHooks({ args: mochaHookArgs, hookName: mochaHookName });
    }

    function lintDescribeBlock(expressionArgs) {
      expressionArgs.forEach((arg) => {
        if (!isTestBlock(arg.type)) { return; }
        if (!_.get(arg, 'body.body.type') === 'BlockStatement') { return; }
    ...
    return {
      ExpressionStatement: (node) => {
        const callee = _.get(node, 'expression.callee');
        if (!callee) { return; }
